...
const service = axios.create({
  ...
  baseURL: (<any>import.meta).env.VITE_APP_BASE_API,
  // 超时
  timeout: 10000,
});
...
service.interceptors.response.use(
  (res) => {
    ...
    if (status === 401) {
      ...
      //不为200取后台给的错误信息{code:XXX,message:XXXX}
      ElNotification.error({ title: respondData.message });
      return Promise.reject('error');
    } else {
      return Promise.resolve(respondData);
    }
  },
  (error) => {
...
// 通用下载方法
export function download(url: any, params: any, filename: string, config: any) {
